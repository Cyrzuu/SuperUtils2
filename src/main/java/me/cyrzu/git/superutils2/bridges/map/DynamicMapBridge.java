...
public class DynamicMapBridge implements MapBridge {
    ...
    private Object getValueByPath(String[] parts, int index) {
        ...
        if (next instanceof DynamicMapBridge dynamicMapBridge) {
            return dynamicMapBridge.getValueByPath(parts, index + 1);
        }
    ...
    private MapBridge getOrCreateNestedMap(String[] parts, int index) {
        ...
        if (next instanceof DynamicMapBridge dynamicMapBridge) {
            ...
            return dynamicMapBridge.getOrCreateNestedMap(parts, index + 1);
        } else {
